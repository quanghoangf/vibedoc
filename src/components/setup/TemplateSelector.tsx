...
function groupByCategory(
  templates: Template[],
): { category: TemplateCategory; displayName: string; items: Template[] }[] {
...
export function TemplateSelector({
  ...
  const toggle = (item: Template) => {
    if (isSelected(item.id)) {
      ...
      onChange([
        ...selected,
        { id: item.id, name: item.name, path: item.defaultPath },
      ]);
    }
  };
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {group.items.map((item) => {
                  const Icon =
                    TEMPLATE_ICONS[item.id] ?? CATEGORY_ICONS[item.category];
